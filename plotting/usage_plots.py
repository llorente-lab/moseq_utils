...
def run_kw_dunn(data, group_col, value_col, control_group):
    ...
    for syll in syllables:
        ...
        group_data = [
            syll_data[syll_data[group_col] == group][value_col] for group in groups
        ]
...
def plot_syllable_usage_comparison(
    stats_df,
    group1_name,
    group2_name,
    mapping,
    ...
    syllables = pd.Index(
        set(group1["syllable"].unique()) | set(group2["syllable"].unique())
    )
    diff = group2.groupby("syllable")["usage"].mean().reindex(syllables).fillna(
        0
    ...
    ax.set_title(
        f"Syllable Usage Comparison: {group1_name} vs {group2_name}", fontsize=14
    )
    ...
    plt.scatter(
        markings, [ax.get_ylim()[1]] * len(markings), color="r", marker="*", s=100
    )
    ...
    handles = [
        mlines.Line2D(
            [], [], color=color, marker="o", linestyle="None", markersize=9, label=group
        )
...
def jensen_shannon_divergence(
    stats_df, mapping, groups=["all"], save_path="jsd_matrix.png"
):
    """
    ...
    grouped_df["probability"] = grouped_df.groupby("group")["usage"].transform(
        lambda x: x / x.sum()
    )
    pivot_df = grouped_df.pivot(
        index="syllable", columns="group", values="probability"
    ).fillna(0)
    ...
    ax1.set_title(
        "Jensen-Shannon Divergence between Group Syllable Usage Distributions",
        fontsize=16,
    )
